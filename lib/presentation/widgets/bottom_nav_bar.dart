...
class BottomNavBar extends StatelessWidget {
  ...
  static const _items = [
    ...
    _NavItemData(icon: Icons.search_outlined,    activeIcon: Icons.search_rounded,    label: 'Search'),
    _NavItemData(icon: Icons.favorite_outline,   activeIcon: Icons.favorite_rounded,  label: 'Saved'),
  ];
  ...
  void _onTap(BuildContext context, int index) {
    switch (index) {
      ...
      case 2: context.go(AppRoutes.favorites); break;
    }
  }
...
class _WavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(_buildPath(size, cx, offsetY: 0), fillPaint);
  }
  ...
  Path _buildPath(Size size, double cx, {double offsetY = 0}) {
    ...
    final path = Path()
      ...
      ..cubicTo(
        left - 8, top,
        left,     top + notchDepth * 0.55,
        cx - notchR * 0.65, top + notchDepth,
      )

      // Bottom arc of the notch
      ..cubicTo(
        cx - notchR * 0.2, top + notchDepth + 6,
        cx + notchR * 0.2, top + notchDepth + 6,
        cx + notchR * 0.65, top + notchDepth,
      )
